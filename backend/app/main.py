...
# #     pdf_to_images(PDF_PATH, f"{OUTPUT_DIR}/images")
...
# #     print(f"✅ Done. Pages processed: {len(pages)} | Chunks created: {len(all_chunks)}")
...
#         chunks = chunk_text(cleaned, page["page"])
...
#     print(
#         f"✅ Done. Pages processed: {len(pages)} | "
...
from ingestion.chunker import chunk_text
from ingestion.text_cleaner import clean_text, is_noise
from ingestion.enricher import enrich_chunks  # <--- Import this
...
def run_ingestion():
    ...
    # --- Step 1: Images ---
    print("📄 Extracting images...")
    # pdf_to_images(PDF_PATH, f"{OUTPUT_DIR}/images") # Uncomment if needed
    ...
    for page in pages:
        ...
        chunks = chunk_text(cleaned, page=page["page"]) # Pass page arg
        raw_chunks.extend(chunks)

    # --- Step 4: Enrichment (Week 2 Goal) ---
